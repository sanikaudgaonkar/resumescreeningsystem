...
def query_top_k_chunks_for_requirement(resume_id: str, requirement_embedding: list[float], k: int = 5) -> list[dict]:
    ...
    for doc, distance in zip(result["documents"][0], result["distances"][0]):
        ...
        candidates.append({"chunk_text": doc, "score": similarity})
    return candidates
